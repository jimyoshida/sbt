...
- **`sbt sync <src> <dst>`** — incremental/full directory backups using `rsync`. Creates timestamped snapshot directories (`backup-YYYYMMDD-HHMMSS`). Supports hardlink-based incremental backups via rsync's `--link-dest`. Pass `--full` to force a full backup.
- **`sbt archive <dir> <volname>`** — creates ISO 9660 images from a directory using `mkisofs` (provided by `genisoimage`). Outputs `<VOLNAME>.iso` in the current directory, with Joliet and Rock Ridge extensions for cross-platform compatibility.
